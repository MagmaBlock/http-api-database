...
export async function cdnGetImage(req, res) {
  ...
  if (requestPath.endsWith(");")) {
    console.log(
      chalk.bgYellowBright(" 修复异常后缀 "),
      chalk.bgGrey(req.originalUrl)
    );
...
async function isNSFW(subjectID) {
  try {
    let isInDB = await promiseDB.query(
      "SELECT * FROM nsfw WHERE subject_id = ?",
      [subjectID]
    );
    if (isInDB[0].length && isInDB[0][0]?.create_time) {
      // 数据库中找到相关数据
      let data = isInDB[0][0];
      ...
      if (expiredTime > new Date()) {
        ...
        return {
          ...
          score: data.score, // 如果分数大于等于 8, 将会建议屏蔽
