...
export async function onlineReportAPI(req, res) {
  ...
  try {
    ...
    let thisUserInDB = await promiseDB.query(
      "SELECT * FROM online WHERE user_id = ?",
      [userID]
    );
    if (thisUserInDB[0].length > 0) {
      // 此用户已存在
      let updateQuery = await promiseDB.query(
        "UPDATE online SET last_online = ? WHERE user_id = ?",
        [new Date(), userID]
      );
      ...
      let insertQuery = await promiseDB.query(
        "INSERT INTO online (user_id, last_online) VALUES (?, ?)",
        [userID, new Date()]
      );
